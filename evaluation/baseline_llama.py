...
def few_shot(model: AutoModelForCausalLM, tokenizer: AutoTokenizer, doc: str, claim: str, answer: bool, examples: List[Dict[str, str]], cot: bool) -> int:
    if len(examples) == 0:
        return zero_shot(model, tokenizer, doc, claim, answer)
...
def main():
    ...
    for data in test:
        ...
        if NUM_EXAMPLE > 0:
            ...
            #print(examples)
            answer  = few_shot(model, tokenizer, doc, claim, label, examples, COT)
            results.append({
